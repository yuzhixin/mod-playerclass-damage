...
public:
	...
	void LoadFromConfig()
	{
		...
		LOG_INFO("mod_class_damage_regulator", "Loading class damage modifiers...");
		...
		for (auto const &[name, id] : ClassNameToId)
		{
			...
			LOG_DEBUG("mod_class_damage_regulator", "  %s: %.1f%%", name.c_str(), val);
		}

		_classModifiers = modifiers;
		LOG_INFO("mod_class_damage_regulator", "Successfully loaded %u class damage modifiers", modifiers.size());
	}
...
public:
	...
	void OnStartup() override
	{
		...
		LOG_INFO("mod_class_damage_regulator", "Version: 1.1.0 | Author: YourName");
		sClassDamageRegulator->LoadFromConfig();
		LOG_INFO("mod_class_damage_regulator", "Module loaded successfully");
	}
};
